...
if ($_POST["vercode"] != $_SESSION["vercode"] OR $_SESSION["vercode"]=='')  { 
     echo  "<span style='color:white;'><strong>Incorrect verification code.</strong><br />
Please use back button to avoid loss of submited data </span>";
} 
...
$copy = copy($_FILES['ufile']['tmp_name'][0], $path);
 ...
 if($copy){
 echo "<span style='color:white;'>Autor image was uploaded sucessfully!</span><br>";
 }else{
 die ( "<span style='color:white;'>Autor image could not be uploaded!</span><br>");
 }
//copy file to where you want to store file
$copy = copy($_FILES['ufile']['tmp_name'][1], $path1);
 // prompt if successfully copied
 if($copy){
 echo "<span style='color:white;'>Still 1 image was uploaded sucessfully!</span><br>";
 }else{
 die ( "<span style='color:white;'>Still 1 image could not be uploaded!</span><br>");
 }
 //copy file to where you want to store file
$copy = copy($_FILES['ufile']['tmp_name'][2], $path2);
 // prompt if successfully copied
 if($copy){
 echo "<span style='color:white;'>Still 2 image was uploaded sucessfully!</span><br>";
 }else{
 die ( "<span style='color:white;'>Still 2 image could not be uploaded!</span><br>");
 }
...
$query2 = "INSERT into prijave (id,oriTitle,engTitle,autor,country,region,year,duration,format,screenplay,dop,editing,actors,music,other,production,paddress,ptel,pfax,pemail,tel,link,link_password,email,sinopsis,slikaAutora,slikaFilm1,slikaFilm2,bioFilmography,vreme,year_of_festival,address) values (".$id.",'".$oriTitle." ','".$engTitle."','".$autor."','".$country."','".$region."',".$year.",'".$duration ."','".$format." ','".$screenplay."','".$dop."','".$editing."','".$actors."','".$music."','".$other."','".$production."','".$paddress."','".$ptel."','".$pfax."','".$pemail."','".$tel."','".$link."','".$link_password."','".$email."','".$sinopsis."','".$path."','".$path1."','".$path2."','".$bioFilmography."','".$vreme."',".$year_of_festival.",'".$address."');";
//echo $query2;
$result2 = mysql_query($query2);
mysql_error($con); 
if(!$result2){
	die("
	  <span style='color:white; background-color:black;'><br />
      <br />
      Erorr No. 2. Please notify technical support, email:it@dksg.rs</span></a>");
}
...
<br />
<br />
Application was successfully submited.<br />
<br />
<br />
<br />
 </body>
...
//ako slike nisu selektovane ili su prevelike
	...
	die("<span style='color:white; background-color:black;'>You didn't selected images or you selected images over 1MB. <br />
         <br />
         Use return button of yuor browser to go back to form.</span>");
	}
}// ako je u redu captcha
 ?>
